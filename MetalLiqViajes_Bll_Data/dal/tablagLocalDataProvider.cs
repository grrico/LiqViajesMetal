...
{
	/// <summary>
	/// Data Access Layer class for tablagLocal object
	/// </summary>
	/// <remarks>
	/// This class is a Singleton
	/// </remarks>
	public partial class tablagLocalDataProvider
	{
		/// <summary>
		/// Internal member for create this singleton
		/// </summary>
		private static tablagLocalDataProvider MySingleObj = null;

		/// <summary>
		/// Protected constructor to avoid access this singleton other way than calling the Instance method
		/// </summary>
		public tablagLocalDataProvider()
		{
		...
		/// </summary>
		public static tablagLocalDataProvider Instance
		{
			get
			{
				if (MySingleObj == null)
				{
					MySingleObj = new tablagLocalDataProvider();
				}
		...
		/// <summary>
		/// Creates a new record into tablagLocal by passing all fields
		/// </summary>
		/// <param name="ano"></param>
		/// <param name="periodo"></param>
		/// <param name="fecha"></param>
		public void Create(int Codigo, int? ano, int? periodo, DateTime? fecha,string module, Sinapsys.Datos.SQL datosTransaccion=null)
		{
			try 
			{
				...
				LocalDataProvider.EjecutarProcedimiento("dbo.tablagLocalCreate", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
		...
		public void Update(int Codigo, int? ano, int? periodo, DateTime? fecha,string module, Sinapsys.Datos.SQL datosTransaccion=null)
		{
			try 
			{
				...
				LocalDataProvider.EjecutarProcedimiento("dbo.tablagLocalUpdate", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Deletes one record from tablagLocal by passing all key fields
		/// </summary>
		/// <param name="Codigo"></param>
		public void Delete(int Codigo,string module, Sinapsys.Datos.SQL datosTransaccion=null)
		{
			try 
			{
				...
				System.Collections.Hashtable nullExit = null;
				paramlist.AddWithValue("@Codigo",Codigo);
				LocalDataProvider.EjecutarProcedimiento("dbo.tablagLocalDelete", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Gets one record from tablagLocal passing all key fields
		/// </summary>
		/// <param name="Codigo"></param>
		...
		public DataTable Get(int Codigo)
		{
			try 
			{
				...
				System.Collections.Hashtable nullExit = null;
				paramlist.AddWithValue("@Codigo",Codigo);
				return LocalDataProvider.EjecutarProcedimiento("dbo.tablagLocalGet", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
		...
		/// <summary>
		/// Gets all records from tablagLocal
		/// </summary>
		...
		public DataTable GetAll()
		{
			try 
			{
				...
				System.Data.SqlClient.SqlParameterCollection paramlist = Comando.Parameters;
				System.Collections.Hashtable nullExit = null;
				return LocalDataProvider.EjecutarProcedimiento("dbo.tablagLocalGetAll", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Gets all records from tablagLocal applying filter and sort criteria
		/// </summary>
		...
		public DataTable GetFilter(int pagenum, int pagesize, string filter, string sort, string extablesfilter, string extablesfieldsfilter, string extablesrelationsfilter, string extablessort, string extablesfieldssort, string extablesrelationssort)
		{
			try 
			{
				...
				return LocalDataProvider.EjecutarProcedimiento("dbo.tablagLocalGetFilter", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Gets the numbers of records from tablagLocal applying filter and sort criteria
		/// </summary>
		/// <param name="pagenum">Contents the page number (z-ordered) of records to return.</param>
		/// <param name="pagesize">Contents the number of records per page (0 for returns all records).</param>
		/// <param name="filter">Contents the criteria (as SQL sentence) for filter records (ex. Qty > 0).</param>
		...
		public int GetFilterCount(int pagenum, int pagesize, string filter, string sort, string extablesfilter, string extablesfieldsfilter, string extablesrelationsfilter, string extablessort, string extablesfieldssort, string extablesrelationssort)
		{
			try 
			{
				...
				return (int) LocalDataProvider.EjecutarProcedimiento("dbo.tablagLocalGetFilter", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera).Rows[0][0];
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
