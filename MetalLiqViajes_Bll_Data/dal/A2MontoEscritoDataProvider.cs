...
{
	/// <summary>
	/// Data Access Layer class for A2MontoEscrito object
	/// </summary>
	/// <remarks>
	/// This class is a Singleton
	/// </remarks>
	public partial class A2MontoEscritoDataProvider
	{
		/// <summary>
		/// Internal member for create this singleton
		/// </summary>
		private static A2MontoEscritoDataProvider MySingleObj = null;

		/// <summary>
		/// Protected constructor to avoid access this singleton other way than calling the Instance method
		/// </summary>
		public A2MontoEscritoDataProvider()
		{
		...
		/// </summary>
		public static A2MontoEscritoDataProvider Instance
		{
			get
			{
				if (MySingleObj == null)
				{
					MySingleObj = new A2MontoEscritoDataProvider();
				}
		...
		/// <summary>
		/// Creates a new record into tblA2MontoEscrito by passing all fields
		/// </summary>
		/// <param name="bytTipoCifra"></param>
		/// <param name="bytPosicionCifra"></param>
		/// <param name="strCifra"></param>
		public void Create(byte bytTipoCifra, byte bytPosicionCifra, string strCifra,string module, Sinapsys.Datos.SQL datosTransaccion=null)
		{
			try 
			{
				...
				paramlist.AddWithValue("@bytTipoCifra",bytTipoCifra);
				paramlist.AddWithValue("@bytPosicionCifra",bytPosicionCifra);
				if (strCifra !=null)
				{
					paramlist.AddWithValue("@strCifra",strCifra);
				}
				LocalDataProvider.EjecutarProcedimiento("dbo.A2MontoEscritoCreate", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
		...
		/// <summary>
		/// Gets all records from tblA2MontoEscrito
		/// </summary>
		...
		public DataTable GetAll()
		{
			try 
			{
				...
				System.Data.SqlClient.SqlParameterCollection paramlist = Comando.Parameters;
				System.Collections.Hashtable nullExit = null;
				return LocalDataProvider.EjecutarProcedimiento("dbo.A2MontoEscritoGetAll", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Gets all records from tblA2MontoEscrito applying filter and sort criteria
		/// </summary>
		...
		public DataTable GetFilter(int pagenum, int pagesize, string filter, string sort, string extablesfilter, string extablesfieldsfilter, string extablesrelationsfilter, string extablessort, string extablesfieldssort, string extablesrelationssort)
		{
			try 
			{
				...
				return LocalDataProvider.EjecutarProcedimiento("dbo.A2MontoEscritoGetFilter", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Gets the numbers of records from tblA2MontoEscrito applying filter and sort criteria
		/// </summary>
		/// <param name="pagenum">Contents the page number (z-ordered) of records to return.</param>
		/// <param name="pagesize">Contents the number of records per page (0 for returns all records).</param>
		/// <param name="filter">Contents the criteria (as SQL sentence) for filter records (ex. Qty > 0).</param>
		...
		public int GetFilterCount(int pagenum, int pagesize, string filter, string sort, string extablesfilter, string extablesfieldsfilter, string extablesrelationsfilter, string extablessort, string extablesfieldssort, string extablesrelationssort)
		{
			try 
			{
				...
				return (int) LocalDataProvider.EjecutarProcedimiento("dbo.A2MontoEscritoGetFilter", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera).Rows[0][0];
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
