...
{
	/// <summary>
	/// Data Access Layer class for RutasDestino object
	/// </summary>
	/// <remarks>
	/// This class is a Singleton
	/// </remarks>
	public partial class RutasDestinoDataProvider
	{
		/// <summary>
		/// Internal member for create this singleton
		/// </summary>
		private static RutasDestinoDataProvider MySingleObj = null;

		/// <summary>
		/// Protected constructor to avoid access this singleton other way than calling the Instance method
		/// </summary>
		public RutasDestinoDataProvider()
		{
		...
		/// </summary>
		public static RutasDestinoDataProvider Instance
		{
			get
			{
				if (MySingleObj == null)
				{
					MySingleObj = new RutasDestinoDataProvider();
				}
		...
		/// <summary>
		/// Updates one record into RutasDestino by passing all fields
		/// </summary>
		...
		/// <summary>
		/// Gets all records from RutasDestino
		/// </summary>
		...
		public DataTable GetAll()
		{
			try 
			{
				...
				System.Data.SqlClient.SqlParameterCollection paramlist = Comando.Parameters;
				System.Collections.Hashtable nullExit = null;
				return LocalDataProvider.EjecutarProcedimiento("dbo.RutasDestinoGetAll", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Gets all records from RutasDestino that are related to RutasOrigen
		/// </summary>
		/// <param name="RutasOrigenCodigo"></param>
		/// <returns>A DataTable object containing all records data</returns>
		public DataTable GetBy_RutasOrigenCodigo(int RutasOrigenCodigo)
		{
			try 
			{
				...
				paramlist.AddWithValue("@RutasOrigenCodigo",RutasOrigenCodigo);
				return LocalDataProvider.EjecutarProcedimiento("dbo.RutasDestinoGetBy_RutasOrigenCodigo", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Gets all records from RutasDestino applying filter and sort criteria
		/// </summary>
		...
		public DataTable GetFilter(int pagenum, int pagesize, string filter, string sort, string extablesfilter, string extablesfieldsfilter, string extablesrelationsfilter, string extablessort, string extablesfieldssort, string extablesrelationssort)
		{
			try 
			{
				...
				return LocalDataProvider.EjecutarProcedimiento("dbo.RutasDestinoGetFilter", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Gets the numbers of records from RutasDestino applying filter and sort criteria
		/// </summary>
		/// <param name="pagenum">Contents the page number (z-ordered) of records to return.</param>
		/// <param name="pagesize">Contents the number of records per page (0 for returns all records).</param>
		/// <param name="filter">Contents the criteria (as SQL sentence) for filter records (ex. Qty > 0).</param>
		...
		public int GetFilterCount(int pagenum, int pagesize, string filter, string sort, string extablesfilter, string extablesfieldsfilter, string extablesrelationsfilter, string extablessort, string extablesfieldssort, string extablesrelationssort)
		{
			try 
			{
				...
				return (int) LocalDataProvider.EjecutarProcedimiento("dbo.RutasDestinoGetFilter", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera).Rows[0][0];
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
