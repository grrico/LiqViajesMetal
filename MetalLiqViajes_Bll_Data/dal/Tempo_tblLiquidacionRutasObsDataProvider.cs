...
{
	/// <summary>
	/// Data Access Layer class for Tempo_tblLiquidacionRutasObs object
	/// </summary>
	/// <remarks>
	/// This class is a Singleton
	/// </remarks>
	public partial class Tempo_tblLiquidacionRutasObsDataProvider
	{
		/// <summary>
		/// Internal member for create this singleton
		/// </summary>
		private static Tempo_tblLiquidacionRutasObsDataProvider MySingleObj = null;

		/// <summary>
		/// Protected constructor to avoid access this singleton other way than calling the Instance method
		/// </summary>
		public Tempo_tblLiquidacionRutasObsDataProvider()
		{
		...
		/// </summary>
		public static Tempo_tblLiquidacionRutasObsDataProvider Instance
		{
			get
			{
				if (MySingleObj == null)
				{
					MySingleObj = new Tempo_tblLiquidacionRutasObsDataProvider();
				}
		...
		/// <summary>
		/// Creates a new record into tblTempo_tblLiquidacionRutasObs by passing all fields
		/// </summary>
		...
		public void Create(int lngIdRegistrRutaItemId, int? lngIdRegistrRuta, int? lngIdRegistro, char? strCampo, char? strObservacion, char? strMotivo, int? lngIdUsuario,string module, Sinapsys.Datos.SQL datosTransaccion=null)
		{
			try 
			{
				...
				if (strMotivo !=null)
				{
					paramlist.AddWithValue("@strMotivo",strMotivo);
				}
				if (lngIdUsuario !=null)
				{
					paramlist.AddWithValue("@lngIdUsuario",lngIdUsuario);
				}
				LocalDataProvider.EjecutarProcedimiento("dbo.Tempo_tblLiquidacionRutasObsCreate", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
		...
		/// <summary>
		/// Gets all records from tblTempo_tblLiquidacionRutasObs
		/// </summary>
		...
		public DataTable GetAll()
		{
			try 
			{
				...
				System.Data.SqlClient.SqlParameterCollection paramlist = Comando.Parameters;
				System.Collections.Hashtable nullExit = null;
				return LocalDataProvider.EjecutarProcedimiento("dbo.Tempo_tblLiquidacionRutasObsGetAll", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Gets all records from tblTempo_tblLiquidacionRutasObs applying filter and sort criteria
		/// </summary>
		...
		public DataTable GetFilter(int pagenum, int pagesize, string filter, string sort, string extablesfilter, string extablesfieldsfilter, string extablesrelationsfilter, string extablessort, string extablesfieldssort, string extablesrelationssort)
		{
			try 
			{
				...
				return LocalDataProvider.EjecutarProcedimiento("dbo.Tempo_tblLiquidacionRutasObsGetFilter", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Gets the numbers of records from tblTempo_tblLiquidacionRutasObs applying filter and sort criteria
		/// </summary>
		/// <param name="pagenum">Contents the page number (z-ordered) of records to return.</param>
		/// <param name="pagesize">Contents the number of records per page (0 for returns all records).</param>
		/// <param name="filter">Contents the criteria (as SQL sentence) for filter records (ex. Qty > 0).</param>
		...
		public int GetFilterCount(int pagenum, int pagesize, string filter, string sort, string extablesfilter, string extablesfieldsfilter, string extablesrelationsfilter, string extablessort, string extablesfieldssort, string extablesrelationssort)
		{
			try 
			{
				...
				return (int) LocalDataProvider.EjecutarProcedimiento("dbo.Tempo_tblLiquidacionRutasObsGetFilter", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera).Rows[0][0];
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
