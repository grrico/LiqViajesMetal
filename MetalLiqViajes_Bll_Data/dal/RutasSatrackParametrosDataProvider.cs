...
{
	/// <summary>
	/// Data Access Layer class for RutasSatrackParametros object
	/// </summary>
	/// <remarks>
	/// This class is a Singleton
	/// </remarks>
	public partial class RutasSatrackParametrosDataProvider
	{
		/// <summary>
		/// Internal member for create this singleton
		/// </summary>
		private static RutasSatrackParametrosDataProvider MySingleObj = null;

		/// <summary>
		/// Protected constructor to avoid access this singleton other way than calling the Instance method
		/// </summary>
		public RutasSatrackParametrosDataProvider()
		{
		...
		/// </summary>
		public static RutasSatrackParametrosDataProvider Instance
		{
			get
			{
				if (MySingleObj == null)
				{
					MySingleObj = new RutasSatrackParametrosDataProvider();
				}
		...
		/// <summary>
		/// Creates a new record into RutasSatrackParametros by passing all fields
		/// </summary>
		...
		public void Create(long Codigo, string NombreEvento, bool? ACtivoWS, bool? ACtivoWinForm, int? TipoEvento,string module, Sinapsys.Datos.SQL datosTransaccion=null)
		{
			try 
			{
				...
				{
					paramlist.AddWithValue("@NombreEvento",NombreEvento);
				}
				if (ACtivoWS !=null)
				{
					paramlist.AddWithValue("@ACtivoWS",ACtivoWS);
				}
				if (ACtivoWinForm !=null)
				{
					paramlist.AddWithValue("@ACtivoWinForm",ACtivoWinForm);
				}
				if (TipoEvento !=null)
				{
					paramlist.AddWithValue("@TipoEvento",TipoEvento);
				}
				LocalDataProvider.EjecutarProcedimiento("dbo.RutasSatrackParametrosCreate", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
		...
		/// <summary>
		/// Updates one record into RutasSatrackParametros by passing all fields
		/// </summary>
		...
		public void Update(long Codigo, string NombreEvento, bool? ACtivoWS, bool? ACtivoWinForm, int? TipoEvento,string module, Sinapsys.Datos.SQL datosTransaccion=null)
		{
			try 
			{
				...
				LocalDataProvider.EjecutarProcedimiento("dbo.RutasSatrackParametrosUpdate", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		public void Delete(long Codigo,string module, Sinapsys.Datos.SQL datosTransaccion=null)
		{
			try 
			{
				...
				System.Collections.Hashtable nullExit = null;
				paramlist.AddWithValue("@Codigo",Codigo);
				LocalDataProvider.EjecutarProcedimiento("dbo.RutasSatrackParametrosDelete", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Gets one record from RutasSatrackParametros passing all key fields
		/// </summary>
		/// <param name="Codigo"></param>
		...
		public DataTable Get(long Codigo)
		{
			try 
			{
				...
				System.Collections.Hashtable nullExit = null;
				paramlist.AddWithValue("@Codigo",Codigo);
				return LocalDataProvider.EjecutarProcedimiento("dbo.RutasSatrackParametrosGet", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
		...
		/// <summary>
		/// Gets all records from RutasSatrackParametros
		/// </summary>
		...
		public DataTable GetAll()
		{
			try 
			{
				...
				System.Data.SqlClient.SqlParameterCollection paramlist = Comando.Parameters;
				System.Collections.Hashtable nullExit = null;
				return LocalDataProvider.EjecutarProcedimiento("dbo.RutasSatrackParametrosGetAll", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Gets all records from RutasSatrackParametros applying filter and sort criteria
		/// </summary>
		...
		public DataTable GetFilter(int pagenum, int pagesize, string filter, string sort, string extablesfilter, string extablesfieldsfilter, string extablesrelationsfilter, string extablessort, string extablesfieldssort, string extablesrelationssort)
		{
			try 
			{
				...
				return LocalDataProvider.EjecutarProcedimiento("dbo.RutasSatrackParametrosGetFilter", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera);
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
		...
		/// <summary>
		/// Gets the numbers of records from RutasSatrackParametros applying filter and sort criteria
		/// </summary>
		/// <param name="pagenum">Contents the page number (z-ordered) of records to return.</param>
		/// <param name="pagesize">Contents the number of records per page (0 for returns all records).</param>
		/// <param name="filter">Contents the criteria (as SQL sentence) for filter records (ex. Qty > 0).</param>
		...
		public int GetFilterCount(int pagenum, int pagesize, string filter, string sort, string extablesfilter, string extablesfieldsfilter, string extablesrelationsfilter, string extablessort, string extablesfieldssort, string extablesrelationssort)
		{
			try 
			{
				...
				return (int) LocalDataProvider.EjecutarProcedimiento("dbo.RutasSatrackParametrosGetFilter", paramlist, disconnect, out nullExit, DataProvider.TiempoEspera).Rows[0][0];
			}
			catch (Exception ex)
			{
				Utilidades.LogErrores(ex.Message + (ex.InnerException != null ? ex.InnerException.Message : ""), DataProvider.AplicacionNombre);
